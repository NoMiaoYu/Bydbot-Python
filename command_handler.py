...
async def handle_weather_command(command_name: str, args: list, group_id: str, config: Dict[str, Any]) -> None:
    """
    处理天气命令（使用空格分隔参数）
...
async def handle_command(event: Dict[str, Any], config: Dict[str, Any]) -> None:
    ...
    if is_weather:
        ...
        await handle_weather_command(command_name, args, group_id, config)
    else:
